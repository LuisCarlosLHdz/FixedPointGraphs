...
import numpy as np
############################
#Función para convertir un número en formato Q3.28 a decimal
...
q328_to_dec = np.zeros((n,var)) #Crear un arreglo de índices / Create an array of indices
for i in range(var):
    for j in range(n):
        q328_to_dec[j][i] = hex_to_FPDec(df[i][j])
###############################
with open("matriz.txt", "w") as f:
    for fila in q328_to_dec:
...
plt.subplot(2, 2, 1)
plt.plot(q328_to_dec[:, 0], q328_to_dec[:, 1], linestyle='-', color='black', label='Q3,28')
plt.xlabel("x1")
plt.ylabel("x2")
plt.grid(True)
plt.subplot(2, 2, 2)
plt.plot(q328_to_dec[:, 0], q328_to_dec[:, 2], linestyle='-', color='black', label='Q3,28')
plt.xlabel("x1")
plt.ylabel("x3")
plt.grid(True)
plt.subplot(2, 2, 3)
plt.plot(q328_to_dec[:, 1], q328_to_dec[:, 2], linestyle='-', color='black', label='Q3,28')
plt.xlabel("x1")
plt.ylabel("x4")
plt.grid(True)
